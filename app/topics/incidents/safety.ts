...
export function goPrevious(args) {
    const subTopic: ISubTopic = TopicService.getInstance().findSubTopicFromLink("topics/incidents/breakdowns");
    navigationModule.gotoChapters(subTopic);
}

export function goNext(args) {
    console.log("Next...");
    const subTopic: ISubTopic = TopicService.getInstance().findSubTopicFromLink("topics/incidents/warning");
    navigationModule.gotoChapters(subTopic);
}
