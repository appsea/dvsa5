...
export function goPrevious(args) {
    const subTopic: ISubTopic = TopicService.getInstance().findSubTopicFromLink("topics/vehicleLoading/introduction");
    navigationModule.gotoChapters(subTopic);
}

export function goNext(args) {
    const subTopic: ISubTopic = TopicService.getInstance().findSubTopicFromLink("topics/vehicleLoading/towing");
    navigationModule.gotoChapters(subTopic);
}
