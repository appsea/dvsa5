...
export function goPrevious(args) {
    const subTopic: ISubTopic = TopicService.getInstance().findSubTopicFromLink("topics/documents/introduction");
    navigationModule.gotoChapters(subTopic);
}

export function goNext(args) {
    const subTopic: ISubTopic = TopicService.getInstance().findSubTopicFromLink("topics/documents/insurance");
    navigationModule.gotoChapters(subTopic);
}
