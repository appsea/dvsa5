...
export function onActivityBackPressedEvent(args: AndroidActivityBackPressedEventData) {
    navigationModule.gotoSubtopics("Rules of the road");
    args.cancel = true;
}

export function goPrevious(args) {
    const subTopic: ISubTopic = TopicService.getInstance().findSubTopicFromLink("topics/rules/introduction-page");
    navigationModule.gotoChapters(subTopic);
}

export function goNext(args) {
    const subTopic: ISubTopic = TopicService.getInstance().findSubTopicFromLink("topics/rules/lanes-page");
    navigationModule.gotoChapters(subTopic);
}
