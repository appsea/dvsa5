...
import { ISubTopic, SubTopic } from "./../../shared/questions.model";
...
export function onActivityBackPressedEvent(args: AndroidActivityBackPressedEventData) {
    navigationModule.gotoSubtopics("Alertness");
    args.cancel = true;
}

export function goPrevious(args) {
    const subTopic: ISubTopic = TopicService.getInstance().findSubTopicFromLink("topics/alertness/anticipation");
    navigationModule.gotoChapters(subTopic);
}

export function goNext(args) {
    const subTopic: ISubTopic = TopicService.getInstance().findSubTopicFromLink("topics/alertness/distraction");
    navigationModule.gotoChapters(subTopic);
}
